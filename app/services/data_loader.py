...
class DataLoader:
    ...
    def __init__(self, data_file: Optional[Path] = None):
        ...
        if data_file is None:
            ...
            data_file = base_dir / "data" / "grain_data_wms_format.json"
    ...
    def _build_index(self):
        ...
        for house_code in self.indexed_data:
            self.indexed_data[house_code].sort(
                key=lambda x: x.get("check_time") or x.get("record_time") or ""
            )
    ...
    def query(
        ...
        if start_time or end_time:
            ...
            for record in records:
                check_time_str = (
                    record.get("check_time") or record.get("record_time") or ""
                )
        ...
        for record in records:
            try:
                # 兼容 check_time / record_time 两种字段名
                check_time = record.get("check_time") or record.get("record_time") or ""
                grain_data = GrainTempData(
                    house_code=record.get("house_code", ""),
                    house_name=record.get("house_name", ""),
                    depot_name=record.get("depot_name", ""),
                    check_time=check_time,
                    max_temp=_f(record.get("max_temp")),
                    min_temp=_f(record.get("min_temp")),
                    avg_temp=_f(record.get("avg_temp")),
                    indoor_temp=_f(record.get("indoor_temp")),
                    indoor_humidity=_f(record.get("indoor_humidity")),
                    outdoor_temp=_f(record.get("outdoor_temp")),
                    outdoor_humidity=_f(record.get("outdoor_humidity")),
