...
from datetime import datetime, timedelta
from app.models.domain import Warehouse, Silo, Sensor, Reading, WarehouseInfo, GrainTempData, GasConcentrationData
...
class WMSClient:
    def __init__(self):
        ...
        if self.use_real_data:
            logger.info(f"WMSClient 已加载本地测试数据 {len(self.data_loader.raw_data)} 条记录")
        else:
    ...
    def get_connected_silos(self) -> List[Dict[str, str]]:
        ...
        # 兜底：返回本地已知的仓房
        return [
            {"house_code": "91620702MADKWU312X01001", "house_name": "西北库区 P1", "short_name": "P1"},
            {"house_code": "91620702MADKWU312X01012", "house_name": "西北库区 Q1", "short_name": "Q1"}
        ]
    ...
    def get_warehouse_info(self, house_code: str) -> WarehouseInfo:
        ...
        try:
            ...
            response = self.client.get(WMS_ENDPOINT_WAREHOUSE_INFO, params={"house_code": house_code})
            ...
            logger.warning(f"WMS API 返回异常 (Status: {response.status_code})，尝试回退到本地数据")
        except Exception as e:
        ...
        try:
            ...
            if info_dict:
                # 补全缺失的必填字段（如果本地数据也不全）
                if "house_name" not in info_dict: info_dict["house_name"] = f"{house_code}号仓"
                if "depot_name" not in info_dict: info_dict["depot_name"] = "本地模拟库区"
    ...
    def get_grain_temperature(self, house_code: str, start_time: datetime, end_time: datetime) -> List[GrainTempData]:
        """
            ...
            if response.status_code == 200:
                ...
                if len(api_results) > 0:
                    ...
                    for item in api_results:
                        formatted_results.append(GrainTempData(
                            house_code=item.get("house_code", house_code),
                            house_name=item.get("house_name") or silo_meta.get("house_name") or f"{house_code}号仓",
                            depot_name=item.get("depot_name") or "西北库区",
                            check_time=self._parse_api_date(item.get("check_time", "")),
                            max_temp=item.get("max_temp") if item.get("max_temp") is not None else 0.0,
                            min_temp=item.get("min_temp") if item.get("min_temp") is not None else 0.0,
                            avg_temp=item.get("avg_temp") if item.get("avg_temp") is not None else 0.0,
                            indoor_temp=item.get("indoor_temp") if item.get("indoor_temp") is not None else 0.0,
                            indoor_humidity=item.get("indoor_humidity") if item.get("indoor_humidity") is not None else 0.0,
                            outdoor_temp=item.get("outdoor_temp") if item.get("outdoor_temp") is not None else 0.0,
                            outdoor_humidity=item.get("outdoor_humidity") if item.get("outdoor_humidity") is not None else 0.0,
                            temp_values=item.get("temp_values", "")
                        ))
                    return formatted_results
            logger.warning(f"WMS API 粮温接口无数据或异常 (Status: {response.status_code})，尝试回退")
        except Exception as e:
    ...
    def get_gas_concentration(self, house_code: str, start_time: datetime, end_time: datetime) -> List[GasConcentrationData]:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(api_results) > 0:
                    ...
                    formatted_results = []
                    for item in api_results:
                        formatted_results.append(GasConcentrationData(
                            house_code=item.get("house_code", house_code),
                            house_name=item.get("house_name") or silo_meta.get("house_name") or f"{house_code}号仓",
                            depot_name=item.get("depot_name") or "西北库区",
                            ...
                            check_time=self._parse_api_date(item.get("check_time", "")),
                            sample_points=item.get("sample_points") if item.get("sample_points") is not None else 0,
                            avg_o2=item.get("avg_o2") if item.get("avg_o2") is not None else 0.0,
                            avg_ph3=item.get("avg_ph3") if item.get("avg_ph3") is not None else 0.0,
                            avg_co2=item.get("avg_co2") if item.get("avg_co2") is not None else 0.0,
                            avg_n2=item.get("avg_n2") if item.get("avg_n2") is not None else 0.0,
                            avg_other=item.get("avg_other") if item.get("avg_other") is not None else 0.0,
    ...
    def get_silo(self, silo_id: str) -> Silo:
        ...
        if str(silo_id).startswith("1"):
            sensors = [
                Sensor(id="1-T1", type="temperature", location={"x": 0.0, "y": 0.0, "z": 0.0}),
                Sensor(id="1-T2", type="temperature", location={"x": 5.0, "y": 0.0, "z": 0.0}),
                Sensor(id="1-T3", type="temperature", location={"x": 10.0, "y": 0.0, "z": 0.0}),
                Sensor(id="1-T4", type="temperature", location={"x": 0.0, "y": 5.0, "z": 0.0}),
                Sensor(id="1-T5", type="temperature", location={"x": 5.0, "y": 5.0, "z": 0.0}),
                Sensor(id="1-H1", type="humidity", location={"x": 2.5, "y": 2.5, "z": 0.0}),
            ]
        else:
            sensors = [
                Sensor(id=f"{silo_id}-T1", type="temperature", location={"x": 1.0, "y": 1.0, "z": 1.0}),
                Sensor(id=f"{silo_id}-H1", type="humidity", location={"x": 1.0, "y": 1.0, "z": 1.0})
            ]
    ...
    def get_readings(self, silo_id: str, start_time: datetime, end_time: datetime) -> List[Reading]:
            ...
            while current <= end_time:
                ...
                for sensor_id in ["1-T1", "1-T2", "1-T3", "1-T4", "1-T5"]:
                    if sensor_id == "1-T3":  # 热点传感器
                        # 热点温度：偏高且有波动
                        temp = self.silo_1_temp_pattern["hotspot_temp"] + random.uniform(-1.0, 2.0)
                    else:
                        # 正常温度：基础温度 + 小波动
                        temp = self.silo_1_temp_pattern["base_temp"] + random.uniform(-2.0, 2.0)
