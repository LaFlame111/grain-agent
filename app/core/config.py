...
class Settings:
    ...
    EXPOSE_DEBUG_ENDPOINTS: bool = os.getenv("EXPOSE_DEBUG_ENDPOINTS", "false").lower() == "true"
    ...
    RAGFLOW_DATASET_IDS: str = os.getenv("RAGFLOW_DATASET_IDS", "")  # 逗号分隔多个 dataset id
    ...
    RAG_ENABLE_QUERY_REWRITE: bool = os.getenv("RAG_ENABLE_QUERY_REWRITE", "false").lower() == "true"
    RAG_QUERY_REWRITE_MODEL: str = os.getenv("RAG_QUERY_REWRITE_MODEL", "")  # 空则复用 LLM_MODEL

    def __init__(self):
        # 验证 API Key 是否加载成功
        if not self.DASHSCOPE_API_KEY and not self.EMBEDDING_API_KEY:
            print(f"[WARN] DASHSCOPE_API_KEY / EMBEDDING_API_KEY 均未设置! (检查文件: {env_file})")
        else:
            # 避免在日志里泄露完整密钥，只输出开头/结尾长度
            if self.DASHSCOPE_API_KEY:
                print(f"[OK] 已加载 DASHSCOPE_API_KEY: {self.DASHSCOPE_API_KEY[:6]}...{self.DASHSCOPE_API_KEY[-4:]}")
            if self.EMBEDDING_API_KEY:
                print(f"[OK] 已加载 EMBEDDING_API_KEY: {self.EMBEDDING_API_KEY[:6]}...{self.EMBEDDING_API_KEY[-4:]}")
